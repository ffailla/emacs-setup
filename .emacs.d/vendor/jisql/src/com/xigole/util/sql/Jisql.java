 ...
 * will not be interpreted by the Unix shell). If you do not use this the default
 * is the term "go" on a single line like Sybase's isql or MS/SQL's isql/osql.
 * Note that there is a dependency on
 * <a href="http://jopt-simple.sourceforge.net/">JOpt Simple</a> in for the base
 ...
 * like:
 * <ul>
 * <li><b>jconnect4, jconnect5, jconnect6 </b>- Sybase connection strings take the form
 * &quot;jdbc:sybase:Tds:[hostname]:[port]/[db_name]&quot;</li>
 ...
 * have it visible when looking at a process status.  The first line of the
 * file is read and used as the password.  If both the command line password
 * and this option are specified the command line password is used.</li>
 ...
 * with the command line or a file.  <b>Note</b> - the command <i>must</i>
 * have a command terminator.  So, for example, your command line may be
 * something like &quot;-c \; -query "select * from blah;".  If you do
 * not include the command terminator then the command will hang, waiting
 * for you to enter the default "go".</li>
...
public class Jisql {
	...
	public void run() throws Exception {
		...
		try {
			...
			if (connection != null)
				try { connection.close(); } catch (SQLException ignore) { /* ignored */ }
		}
	}

	/**
	 * The main loop for the Jisql program.  This method handles the input from either
	 * a command line or from a file.  Output is handled through the Formatter.
	 * 
	...
	public void doIsql() throws SQLException {
		...
		while (true) {
			...
			try {
				...
				if (inputFileName != null || printDebug)
	...
	/**
	 * Parse the command line arguments.  This method parses what is needed for the Jisql driver
	 * program and lets the configured formatter do the same.
	 * 
	...
	public void parseArgs(String argv[]) throws Throwable {
		//
		// I'm sure that there has to be a better way but I couldn't find a command line
		// parser that would let me ignore unknown arguments.  so walk through the
		// list once to find the formatter.  then, use the command line parser to
		// do it "for real"
		//
		...
		if ((password == null) && (passwordFileName == null) ) {
			...
			try {
				...
				if( reader != null )
					try { reader.close(); } catch( Exception ignore ) { /* ignored */ }
			}
		}
	...
	private void printAllExceptions(SQLException sqle) {
		while (sqle != null) {
			System.err.println("SQLException : " + "SQL state: "
					+ sqle.getSQLState() + " " + sqle.toString()
	...
	/**
	 * Prints out the usage message for the Jisql driver and the configured formatter.
	 * 
	 */
	private void usage() {
		System.err.println();
		System.err.println( "Jisql v" + Version.version + " (built " + Version.buildDate + ") copyright (c) 2010 Scott Dunbar (scott@xigole.com)" );
		System.err.println("usage: java " + getClass().getName()
                    + " -driver driver -cstring connect_string -user|-u username -password|-p password [-pf password_file] [-c command_term] [-input file_name] [-debug] [-driverinfo] [-formatter formatter]");
